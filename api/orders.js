  ...
  try {
    ...
    if (req.method === "POST") {
      ...
      const productsRes = await pool.query(
        `SELECT id, title, price, stock FROM products WHERE id = ANY($1::int[])`,
        [ids]
      );
      ...
      try {
        ...
        try {
          await sendResendEmail({
            ...
            subject: `New Order #${order.id} - VELORE`,
            html: `
              <h2>New Order</h2>
          ...
          await sendResendEmail({
            to: order.email,
            subject: `Your order #${order.id} - VELORE`,
            html: `
      ...
      if (id) {
        ...
        if (sendCancelEmail && order.email) {
          try {
            await sendResendEmail({
              to: order.email,
              subject: `Order #${order.id} canceled - VELORE`,
              html: `
            ...
            console.log("CANCEL EMAIL ERROR:", emailError);
          }
        }
      } else {
